        ...
        if(type==1){
           ...
           var img=$('.selected222').find('img').attr('src')
            $.post('/user/ad/addAdApply', {
                ...
                if(data.data!="ERROR"){
                    //location.href="adadmin.html"
                    var moneyss=parseFloat($("#fabuMoney").text())
                    window.location.href = "../service/Center.html?&moneyss=" + moneyss + "&info_id=" + AdApply_id2 + "&type=3&title=付款#pagePayment";
                }
            })
        }else {
            var store_id = 12
